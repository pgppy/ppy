...
(function () {
    ...
    const NATIVE_STORE = 'hoc_native_pay';
    try {
        if (sessionStorage.getItem(NATIVE_STORE) === '1') _userPickedNative = true;
    } catch (e) { /* ignore */ }
    ...
    function pickNative() {
        ...
        try { sessionStorage.setItem(NATIVE_STORE, '1'); } catch (e) { /* ignore */ }
    }

    function pickInstant() {
        _userPickedNative = false;
        try { sessionStorage.removeItem(NATIVE_STORE); } catch (e) { /* ignore */ }
    }
...
<div class="hoc-qris-box" id="${PANEL_ID}">
  ...
  <div class="qris-result" id="qrisResultContainer">
    ...
    async function injectOnce() {
        ...
        else deactivateInstant();
        console.log(LOG, 'tab ready. bars=', bars.length, 'QRISAUTO=', !!findQrisAutoRaw(), 'native=', _userPickedNative);
        return true;
    }
