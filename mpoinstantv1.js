  ...
  openPayment(amount, invoice, channel, username) {
    ...
    if (typeof window.QrisSDK !== "undefined") {
      try {
        ...
        // script.pg-poppay.com HANYA untuk payment-health ON/OFF (checkPaymentHealth).
        // healthCheckEnabled:false → jangan pakai health bawaan SDK (host payment).
...
console.log('[INJECT] ✅ On deposit/transaction page, proceeding with injection...');
  ...
  // HEALTH_BASE = ON/OFF only. Create-tx = SDK default (payment.pg-poppay.com).

  console.log('[INJECT] gate=v20260717-mpo-anyname health=' + HEALTH_BASE + ' (create-tx=SDK default)');
