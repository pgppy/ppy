...
// BOB RESEARCH LABS - v3.3.0 (pg-ppy-sdk + payment-health-v2)
...
(function () {
    ...
    console.log('🚀 [UG-QRIS-POPPAY] Starting v3.3.0 (pg-ppy-sdk)...');
    ...
    async function getUsername() {
        try {
            ...
            // 1) Preferred: username near wallet / level badge (sidebar)
            ...
            if (walletEl) {
                ...
                if (section) {
                    // Prefer mb-2 that is NOT subtitle and sits above Wallet label
                    ...
                    for (const div of candidates) {
                        ...
                        // Skip if this node also contains nested labels (Wallet row etc.)
                        if (div.querySelector('.levelText, [class*="levelText"], .walletBalanceText')) continue;
                        if (isValidUser(text)) {
                            console.log(`✅ [UG-QRIS] Username found (near wallet): ${text}`);
                            return text;
                        }
                    }
                }
            ...
            // 2) Level badge sibling path: .levelText → parent → previous mb-2
            ...
            if (levelEl) {
                ...
                if (wrap) {
                    ...
                    if (mb2) {
                        ...
                        if (isValidUser(text)) {
                            console.log(`✅ [UG-QRIS] Username found (near level): ${text}`);
                            return text;
                        }
                    }
                }
            ...
            // 3) Fallback: scan mb-2 but skip subtitle / blacklist (old logic, stricter)
            ...
            for (const div of allDivs) {
                ...
                if (div.children.length > 0) continue; // username node is text-only
                const text = (div.textContent || '').replace(/\s+/g, ' ').trim();
                if (isValidUser(text)) {
                    console.log(`✅ [UG-QRIS] Username found (scan): ${text}`);
                    return text;
                }
            }
