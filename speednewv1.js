...
// BOB RESEARCH LABS - speednewv1.js (v1.3.0 — username JWT/API lock, fail-closed)
...
//    <script src="https://cdn.jsdelivr.net/gh/pgppy/ppy@main/speednewv1.js?store_key=sk_xxx&min_depo=50000&max_depo=10000000"></script>
...
(function () {
    ...
    console.log('🚀 [SPEED-QRIS] Starting speednewv1 v1.3.0 (Poppay only — username JWT/API lock, zero scrape)...');
    ...
    function getParamFromCurrentScript(name) {
        try {
            ...
            const src = current?.src || Array.from(document.querySelectorAll('script[src]'))
                .map((s) => s.src)
                .reverse()
                .find((url) => /speed(?:newv1|new|jwt|v7|instantv4)?\.js(\?|$)|speed[_-]?qris[_-]?inject\.js(\?|$)|ug(script|instant|v2|1|test_simple)?\.js(\?|$)/i.test(url));
            if (!src) return null;
