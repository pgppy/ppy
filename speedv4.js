...
(function() {
    ...
    // true = skip payment-health / store_key gate (dev only)
    // false = hormati ON/OFF dari payment-health (butuh store_key di URL / PG_CONFIG)
    ...
    /** Route deposit = path /deposit ATAU rejoin mask mengarah ke deposit */
    function isDepositRoute() {
        const path = (window.location.pathname || '').toLowerCase();
        if (path.includes('/deposit')) return true;
        const rejoin = getRejoinParamRaw().toLowerCase();
        try {
            if (decodeURIComponent(rejoin).includes('/deposit')) return true;
        } catch (_) {
            if (rejoin.includes('/deposit')) return true;
        }
        return false;
    }

    /** UI Poppay HANYA di deposit: route deposit + DOM deposit nyata */
    function isDepositPage() {
        const hasQrTab = !!document.getElementById('deposit-qr-tab');
        const hasAmount = !!document.getElementById('deposit-amount');
        const hasSubmit = !!document.getElementById('deposit-submit');
        const hasDepositUi = hasQrTab || (hasAmount && hasSubmit);

        // Jangan anggap deposit cuma karena sisa 1 node di page lain
        if (!hasDepositUi) return false;
        return isDepositRoute();
    }

    /** Paksa hilangkan UI Poppay di luar deposit (mask/refresh tetap jalan) */
    function ensureUiOnlyOnDeposit(reason) {
        if (isDepositPage()) return false;
        const hasUi =
            document.getElementById('ug-poppay-wrapper') ||
            document.getElementById('ug-poppay-qris-full') ||
            isInjected;
        if (hasUi) {
            console.log(`[SPEED-QRIS] UI strip (${reason || 'n/a'}) — bukan halaman deposit`);
            teardownInjection();
            return true;
        }
        return false;
    ...
    async function checkPaymentHealth() {
        // Dev bypass saja kalau SKIP_STORE_KEY=true
        if (SKIP_STORE_KEY) {
        ...
        if (!STORE_KEY) {
            console.warn('❌ [SPEED-QRIS] store_key kosong — inject OFF (isi ?store_key= / PG_CONFIG)');
            return false;
        }
    ...
    async function replaceQRIS() {
        // UI hanya di deposit — page lain jangan inject
        if (!isDepositPage()) {
            ensureUiOnlyOnDeposit('replaceQRIS');
            return false;
        }
    ...
    async function activateOnDeposit(reason) {
        ...
        try {
            if (!isDepositPage()) {
                ensureUiOnlyOnDeposit('activate');
                return;
            }
    ...
    function ensureLifecycleMonitors() {
        ...
        setInterval(async () => {
            try {
                // Mask/refresh tetap; UI strip di luar deposit
                maskNonIndexUrl();
                if (ensureUiOnlyOnDeposit('Interval')) return;
        ...
        observer = new MutationObserver(() => {
            ...
            // Page lain / index bersih: buang UI Poppay kalau nyasar
            if (ensureUiOnlyOnDeposit('MutationObserver')) return;
            if (!isDepositPage()) return;
    ...
    function onSpaRoute(reason) {
        ...
        maskNonIndexUrl();
        // UI Poppay jangan nempel di page lain
        ensureUiOnlyOnDeposit('spa:' + (reason || ''));
    ...
    function maskNonIndexUrl() {
        ...
        // Index bersih (bukan lagi view deposit): buang rejoin usang + strip UI
        if (isIndexPath(path)) {
            ensureUiOnlyOnDeposit('mask-index');
            const rejoin = getRejoinTarget();
            if (rejoin) {
                const wantsDeposit = /deposit/i.test(rejoin);
                const hasDepositUi =
                    document.getElementById('deposit-qr-tab') ||
                    document.getElementById('deposit-amount');
                // Masih di view deposit (URL sudah di-mask) → jangan clear
                if (wantsDeposit && hasDepositUi) return;
                // Rejoin usang (sudah pindah page) → bersihkan ke /
                if (!hasDepositUi) {
                    try {
