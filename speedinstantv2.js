...
// BOB RESEARCH LABS - v1.2.6-speed (username wajib dari players/identity API)
...
(function () {
    ...
    console.log('🚀 [SPEED-QRIS] Starting v1.2.6 (Speed Engine + pre-deposit)...');
    ...
    function getUsernameFromAuthToken() {
        try {
            for (const val of Object.values(parseCookieMap())) {
                ...
                if (!payload || payload.type !== 'player') continue;
    ...
    /** Username untuk Poppay — API session dulu, jangan JWT/localStorage stale. */
    async function resolvePayUsername(options) {
        const force = options?.force === true;
        if (!force && _cachedUsername && isValidUsername(_cachedUsername)) {
        ...
        if (apiUser) {
            ...
            console.log(`✅ [SPEED-QRIS] Username resolved (API): ${apiUser}`);
            return apiUser;
        }
        ...
        if (headerUser) {
            ...
            console.log(`✅ [SPEED-QRIS] Username resolved (header): ${headerUser}`);
            return headerUser;
        }

        const storageUser = getUsernameFromUserInfoStorage();
        if (storageUser) {
            _cachedUsername = storageUser;
            console.log(`✅ [SPEED-QRIS] Username resolved (user-info): ${storageUser}`);
            return storageUser;
        }

        const jwtUser = getUsernameFromAuthToken();
        if (jwtUser) {
            _cachedUsername = jwtUser;
            console.log(`✅ [SPEED-QRIS] Username resolved (jwt): ${jwtUser}`);
            return jwtUser;
        }
    ...
    async function getUsername() {
        try {
            return await resolvePayUsername({ force: false });
        } catch (error) {
    ...
    async function resolvePayUsernameForPayment() {
        _speedProfileCache = null;
        _cachedUsername = '';
        const u = await resolvePayUsername({ force: true });
        if (u) syncInjectUsernameDisplay(u);
        return u;
    }
    ...
    async function fetchSpeedPlayerProfile(username) {
        if (_speedProfileCache?.username && isValidUsername(_speedProfileCache.username)) {
    ...
    async function createSpeedPreDeposit({ username, amount, promotion, playerNote }) {
        ...
        const profile = await fetchSpeedPlayerProfile('');
        const verifiedUser = profile?.username;
        if (verifiedUser && username && verifiedUser.toLowerCase() !== String(username).toLowerCase()) {
            console.warn(`⚠️ [SPEED-QRIS] pre-deposit user mismatch: form=${username} api=${verifiedUser}`);
        }
    ...
    function patchPoppayCreateTransactionBody(opts, payUsername) {
        const forcedUser = (payUsername || '').trim();
        if (!forcedUser || !opts?.body || typeof opts.body !== 'string') return opts;
        try {
            const body = JSON.parse(opts.body);
            body.username = forcedUser;
            body.payor_name = forcedUser;
            const next = Object.assign({}, opts, { body: JSON.stringify(body) });
            console.log('[SPEED-QRIS] create-transaction username patched:', forcedUser);
            return next;
        } catch (e) {
    ...
    function withPoppayCreateTransactionHook(onCreated, payUsername) {
        const origFetch = window.fetch;
        let handled = false;

        window.fetch = async function (...args) {
            const url = String(args[0] || '');
            if (!handled && url.includes('create-transaction') && args[1]) {
                args[1] = patchPoppayCreateTransactionBody(args[1], payUsername);
            }
    ...
    // --- inject global state (must be before replaceQRIS) ---
    let isInjected = false;
    let reinjectionInProgress = false;
    let replaceQRISInProgress = false;
    let injectGuardObserver = null;
    let tryStartTimer = null;

    function getExistingInjectPanel() {
        dedupeInjectWrappers();
        const wrap = document.querySelector('[data-speed-poppay-wrap="true"]');
        if (!wrap) return null;
        const inner = wrap.querySelector('#ug-poppay-qris-full, #speed-poppay-qris-full');
        return inner ? { wrap, inner } : null;
    }

    function dedupeInjectWrappers() {
        const wraps = document.querySelectorAll('[data-speed-poppay-wrap="true"], #ug-poppay-wrapper');
        for (let i = wraps.length - 1; i >= 1; i -= 1) {
            wraps[i].remove();
        }
        document.querySelectorAll('#ug-poppay-qris-full, #speed-poppay-qris-full').forEach((inner) => {
            const wrap = inner.closest('[data-speed-poppay-wrap="true"], #ug-poppay-wrapper');
            if (wrap && wrap !== document.querySelector('[data-speed-poppay-wrap="true"]')) {
                wrap.remove();
            }
        });
    }

    function disconnectInjectGuardObserver() {
        if (injectGuardObserver) {
            injectGuardObserver.disconnect();
            injectGuardObserver = null;
        }
    }

    function startInjectGuardObserver() {
        if (injectGuardObserver) return;
        injectGuardObserver = new MutationObserver(() => {
            if (replaceQRISInProgress || reinjectionInProgress) return;
            if (!isDepositPage() || !isQRISTabActive()) return;

            const panels = document.querySelectorAll('[data-speed-poppay-wrap="true"]');
            if (panels.length > 1) {
                dedupeInjectWrappers();
                return;
            }

            if (getExistingInjectPanel()) {
                isInjected = true;
                return;
            }
            ...
            syncInjectionToQRISTab('Guard').finally(() => {
                reinjectionInProgress = false;
            });
        });
        injectGuardObserver.observe(document.documentElement || document.body, {
            childList: true,
            subtree: true,
        });
    }
    ...
    function findInjectAnchor() {
        ...
        if (amountInput) {
            ...
            if (formBlock?.parentNode) {
                return { parent: formBlock.parentNode, before: formBlock };
            }
        }
        const tabs = document.getElementById('deposit-desktop-tab');
        if (tabs?.parentNode) {
            return { parent: tabs.parentNode, before: tabs.nextSibling };
        }
        const title = Array.from(document.querySelectorAll('b, h1, h2, h3, h4, h5'))
    ...
    function teardownInjection() {
        disconnectInjectGuardObserver();
        document.querySelectorAll('[data-speed-poppay-wrap="true"], #ug-poppay-wrapper').forEach((el) => el.remove());
    ...
    async function syncInjectionToQRISTab(reason) {
        ...
        // Tab QRIS aktif
        dedupeInjectWrappers();
        const existing = getExistingInjectPanel();
        if (existing) {
            hideNativeDepositFormOnQRIS();
            isInjected = true;
            startInjectGuardObserver();
            return true;
        }
    ...
    async function replaceQRIS() {
        ...
        const existingPanel = getExistingInjectPanel();
        if (existingPanel) {
            console.log('ℹ️ [SPEED-QRIS] Already injected (single panel)');
            hideNativeDepositFormOnQRIS();
            startInjectGuardObserver();
            return true;
        }
        ...
        // Use stable container as parent (fallback append only)
        const parentContainer = stableContainer;
        console.log('[SPEED-QRIS] Parent container:', parentContainer);

        const payUser = await resolvePayUsername({ force: true });
        if (!payUser) {
        ...
        // Insert wrapper — single anchor (cegah double panel)
        try {
            const anchor = findInjectAnchor();
            if (anchor?.parent) {
                if (anchor.before && anchor.before.parentNode === anchor.parent) {
                    anchor.parent.insertBefore(wrapper, anchor.before);
                } else {
                    anchor.parent.insertBefore(wrapper, anchor.parent.firstChild);
                }
                console.log('[SPEED-QRIS] Wrapper inserted via findInjectAnchor');
            } else if (parentContainer) {
                parentContainer.insertBefore(wrapper, parentContainer.firstChild);
                console.log('[SPEED-QRIS] Wrapper inserted at container start (fallback)');
            } else {
                throw new Error('No inject anchor');
            }
        } catch (error) {
    ...
    function attachHandlers() {
        ...
        form.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                // Wajib fresh dari players/identity — jangan pakai cache JWT/localStorage
                const payUser = await resolvePayUsernameForPayment();
                if (!payUser) {
    ...
    // ========================================================================
    // Lifecycle monitors (inject state declared above replaceQRIS)
    // ========================================================================
    let observer = null;
    let lifecycleStarted = false;
    let activateInFlight = false;
    let buttonHandlersAttached = false;
    ...
    function onSpaRoute(reason) {
        ...
        clearTimeout(tryStartTimer);
        tryStartTimer = setTimeout(() => tryStart(reason || 'spa'), 300);
    }
