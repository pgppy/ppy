...
(function () {
    ...
    function isValidUser(text) {
        ...
        const blacklist = new Set([
            ...
            'username', 'login', 'register', 'account', 'hotelbet', 'member',
            'instant', 'pulsa', 'ewallet', 'dana', 'gopay', 'ovo',
        ]);
    ...
    function readUsernameFromDom(root) {
        ...
        let username =
            (doc.querySelector('.username') &&
                String(doc.querySelector('.username').innerHTML || '').replace('Hello, ', '').trim()) ||
            (doc.querySelector('#loyalty_level_container > div > a > div > span') &&
                String(doc.querySelector('#loyalty_level_container > div > a > div > span').innerHTML || '').trim()) ||
            '';

        if (username && username.startsWith('<font')) {
            const parsed = new DOMParser().parseFromString(username, 'text/html');
            username = (parsed.body && parsed.body.textContent || '').trim();
            console.log(LOG, 'parsed username:', username);
        }
        return username;
    }
    ...
    async function fetchUsernameFromPage() {
        ...
        const live = readUsernameFromDom(document);
        if (live) {
            const locked = lockUsername(live, '.username live');
            if (locked) return locked;
        }
        return getUsername();
    }
  ...
  <div class="qris-result" id="qrisResultContainer">
    ...
    function bindNativeTabClicks() {
        document.addEventListener('click', function (e) {
            ...
            if (!t) return;
            if (isInstantTab(t)) return;
            const href = channelHref(t);
            const txt = normText(t);
            if (
                href.indexOf('/DEPOSIT') !== -1 ||
                href.indexOf('BANK') !== -1 ||
                href.indexOf('QRIS') !== -1 ||
                href.indexOf('PULSA') !== -1 ||
                href.indexOf('VA') !== -1 ||
                txt.indexOf('BANK') !== -1 ||
                txt.indexOf('QRIS') !== -1 ||
                txt.indexOf('PULSA') !== -1 ||
                t.id === 'qrisauto' ||
                (t.getAttribute && t.getAttribute('for') === 'payment_method_QRISAUTO')
            ) {
                deactivateInstant();
            }
        }, true);
