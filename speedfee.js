...
// BOB RESEARCH LABS - speednewv2.js (v1.5.0 — user-borne fee via merchant-profile)
...
//    Fee: GET https://payment.pg-poppay.com/api/merchant-profile (+ X-Store-Key) → data.fee %
//         QR = amount + fee; robot Speed Engine strip fee sebelum kredit
//    min_depo / max_depo opsional — kalau tidak diisi: default min 10.000 (10rb), max 10.000.000 (10jt)
//    atau PG_CONFIG: window.PG_CONFIG={STORE_KEY:'sk_xxx',MIN_DEPO:10000,MAX_DEPO:10000000,FEE:2.5}
// ============================================================================
...
(function () {
    ...
    console.log('🚀 [SPEED-QRIS] Starting speednewv2 v1.5.0 (user-borne fee + merchant-profile)...');
    ...
    window.speedSetAmount = function (amount, button) {
        ...
        try {
            ...
            if (typeof refreshQrisFeeBreakdown === 'function') refreshQrisFeeBreakdown();
            return false;
        } catch (error) {
    ...
    function patchPoppayCreateTransactionBody(body, payUsername) {
        ...
        try {
            ...
            if (_pendingPayBreakdown && _pendingPayBreakdown.payable > 0) {
                parsed.amount = _pendingPayBreakdown.payable;
            }
    ...
    // ========================================================================
    // Merchant fee (dibebankan ke user) — GET /api/merchant-profile
    // QR payable = credit × (100 + fee) / 100; robot Speed Engine strip balik
    // ========================================================================
    let merchantFeePercent = 0;
    let merchantFeeName = '';
    let merchantFeeFetchedAt = 0;
    let merchantFeeInFlight = null;
    let _pendingPayBreakdown = null;
    const MERCHANT_FEE_CACHE_TTL_MS = 60000;

    function parseFeePercent(raw) {
        if (raw == null || raw === '') return null;
        const n = parseFloat(String(raw).replace(',', '.').replace(/[^\d.+-]/g, ''));
        return Number.isFinite(n) && n >= 0 ? n : null;
    }

    function resolveMerchantFeeOverride() {
        const fromUrl = parseFeePercent(getParamFromCurrentScript('fee'));
        if (fromUrl != null) return fromUrl;
        const cfg = window.PG_CONFIG || {};
        const fromCfg = parseFeePercent(cfg.FEE ?? cfg.fee ?? cfg.MERCHANT_FEE);
        if (fromCfg != null) return fromCfg;
        if (window.PGSCRIPT_FEE != null) {
            const fromWin = parseFeePercent(window.PGSCRIPT_FEE);
            if (fromWin != null) return fromWin;
        }
        return null;
    }

    function applyUserFee(creditAmount) {
        const original = Math.round(Number(creditAmount) || 0);
        const feePercent = Number(merchantFeePercent) || 0;
        if (original <= 0 || feePercent <= 0) {
            return { original, feePercent, feeAmount: 0, payable: original };
        }
        const payable = Math.round(original * (100 + feePercent) / 100);
        return {
            original,
            feePercent,
            feeAmount: Math.max(0, payable - original),
            payable
        };
    }

    function refreshQrisFeeBreakdown() {
        const box = document.getElementById('qrisFeeBreakdown');
        const hidden = document.getElementById('depositAmountAutoQris');
        if (!box) return;
        const credit = parseInt(String(hidden?.value || '').replace(/\D/g, ''), 10) || 0;
        const breakdown = applyUserFee(credit);
        if (!credit || breakdown.feePercent <= 0 || breakdown.feeAmount <= 0) {
        ...
        box.innerHTML =
            `Saldo masuk <strong>Rp ${breakdown.original.toLocaleString('id-ID')}</strong>` +
            ` · Bayar QR <strong>Rp ${breakdown.payable.toLocaleString('id-ID')}</strong>` +
            ` (fee ${breakdown.feePercent}% = Rp ${breakdown.feeAmount.toLocaleString('id-ID')})`;
    }

    async function ensureMerchantFee(force) {
        const override = resolveMerchantFeeOverride();
        if (override != null) {
            merchantFeePercent = override;
            merchantFeeName = merchantFeeName || 'override';
            refreshQrisFeeBreakdown();
        ...
        if (
            !force &&
            merchantFeeFetchedAt &&
            (now - merchantFeeFetchedAt) < MERCHANT_FEE_CACHE_TTL_MS
        ) {
            return merchantFeePercent;
        }
        if (merchantFeeInFlight) return merchantFeeInFlight;

        if (!STORE_KEY) {
            merchantFeePercent = 0;
            merchantFeeFetchedAt = now;
            refreshQrisFeeBreakdown();
            return 0;
        }

        merchantFeeInFlight = (async () => {
            try {
                const res = await fetch(`${PGSCRIPT_BASE}/${PGSCRIPT_API_VERSION}/merchant-profile`, {
                    method: 'GET',
                    cache: 'no-store',
                    headers: {
                ...
                const fee = parseFeePercent(data.fee ?? data.FEE ?? body.fee);
                if (res.ok && (body?.success === true || fee != null)) {
                    merchantFeePercent = fee != null ? fee : 0;
                    merchantFeeName = String(data.name || '').trim();
                    merchantFeeFetchedAt = Date.now();
                    console.log('[SPEED-QRIS] merchant-profile fee:', merchantFeePercent, merchantFeeName || '');
                    refreshQrisFeeBreakdown();
                    return merchantFeePercent;
                }
                console.warn('[SPEED-QRIS] merchant-profile gagal:', body?.message || `HTTP ${res.status}`);
            } catch (err) {
                console.warn('[SPEED-QRIS] merchant-profile error:', err?.message || err);
            } finally {
                merchantFeeInFlight = null;
            }
            merchantFeeFetchedAt = Date.now();
            refreshQrisFeeBreakdown();
            return merchantFeePercent;
                ...
                <div class="qris-form" id="qrisFormContainer">
                    <form id="formDepositAutoQris">
                        ...
                        <div class="form-group mb-3">
                            ...
                            <div class="qris-fee-box" id="qrisFeeBreakdown"></div>
                        </div>
    ...
    function attachHandlers() {
        ...
        form.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                await ensureMerchantFee();
                const breakdown = applyUserFee(amount);
                _pendingPayBreakdown = breakdown;
                ...
                // Invoice: SPEED-{timestamp} — robot hitung kredit dari amount÷(1+fee), bukan invoice
                ...
                console.log('💳 [SPEED-QRIS] Creating payment:', {
                    credit: breakdown.original,
                ...
                const sdkConfig = {
                    ...
                    invoice: invoice,
                    notes: `Speed Auto Deposit - credit=${breakdown.original} fee=${breakdown.feePercent}% pay=${breakdown.payable}`,
                    username: username,
                    payor_name: username,
                    payor_email: '',
                    displayMode: 'inline',
                ...
                sdkConfig.onSuccess = (data) => {
                    ...
                    const feeNote = breakdown.feeAmount > 0
                        ? ` (bayar Rp ${breakdown.payable.toLocaleString('id-ID')} termasuk fee ${breakdown.feePercent}%)`
                        : '';
                    document.getElementById('payment-result').innerHTML = `
                        <div class="ug-qris-success-box">
                            <h4>✅ Pembayaran Berhasil!</h4>
                            <p>Deposit Rp ${breakdown.original.toLocaleString('id-ID')} sedang diproses${feeNote}</p>
                        </div>
                    `;
    ...
    window.SPEED_QRIS_USERNAME_DEBUG = function () {
        ...
        return {
            ...
            fee: {
                percent: merchantFeePercent,
                name: merchantFeeName,
                override: resolveMerchantFeeOverride(),
            },
        };
    };
