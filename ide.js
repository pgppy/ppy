...
// Embed: <script src="...hoc.js?store_key=sk_xxx&min_depo=20000&max_depo=10000000"></script>
...
(function () {
    ...
    const CONFIG = {
        MIN_AMOUNT: 20000,
        MAX_AMOUNT: 10000000,
        AMOUNT_BUTTONS: [20000, 50000, 100000, 200000, 500000, 1000000, 5000000, 10000000],
        MAX_RETRIES: 24,
        RETRY_DELAY: 400,
    ...
    } else if (CONFIG.AMOUNT_BUTTONS.indexOf(CONFIG.MIN_AMOUNT) === -1) {
        CONFIG.AMOUNT_BUTTONS.unshift(CONFIG.MIN_AMOUNT);
    }
    ...
    function panelCss() {
        ...
        return `
#${WRAPPER_ID}{
  --hoc-bg:#151515;--hoc-bg-deep:#080808;--hoc-text:#fff;--hoc-muted:#acacac;
  --hoc-accent:#efbf4d;--hoc-accent-hover:#b88416;--hoc-accent-text:#563b00;
  --hoc-border:#393939;--hoc-btn-bg:#242424;
  --hoc-gold-grad:linear-gradient(to bottom,#f3dd7f 0%,#b88416 100%);
  --hoc-gold-grad-hover:linear-gradient(to bottom,#b88416 0%,#f3dd7f 100%);
  --hoc-shadow:0 4px 18px rgba(0,0,0,.6);z-index:1;position:relative;
}
#${TAB_ID},#${TAB_ID} a,label[for="payment_method_QRISINSTANT"]{cursor:pointer!important}
.hoc-qris-instant-tab a,.hoc-qris-instant-tab label{color:#fae58c!important}
.hoc-qris-instant-tab.active a,.hoc-qris-instant-tab.active label,
.hoc-qris-instant-tab:hover a,.hoc-qris-instant-tab:hover label{color:#fae58c!important}
...
.hoc-qris-box h5{margin:0 0 6px;font-size:${mob ? '16px' : '18px'};color:var(--hoc-accent);font-weight:700}
...
.hoc-qris-box .qris-amount-btn{background:var(--hoc-btn-bg);color:var(--hoc-text);border:1px solid var(--hoc-border);
  border-radius:6px;padding:${mob ? '10px 8px' : '8px 14px'};cursor:pointer;font-size:${mob ? '12px' : '13px'};font-weight:500;transition:all .2s ease}
.hoc-qris-box .qris-amount-btn.active,.hoc-qris-box .qris-amount-btn:hover{
  background:var(--hoc-gold-grad);color:var(--hoc-accent-text);border-color:#b88416;font-weight:700}
.hoc-qris-box .qris-input-group{display:flex;margin-bottom:8px}
.hoc-qris-box .qris-input-prefix{background:var(--hoc-btn-bg);padding:12px;border:1px solid var(--hoc-border);
  border-right:none;border-radius:6px 0 0 6px;color:var(--hoc-accent);font-weight:600}
...
.hoc-qris-box .qris-submit-btn{width:100%;margin-top:12px;padding:14px;
  ...
  color:var(--hoc-accent-text);border:none;border-radius:6px;font-weight:700;cursor:pointer;font-size:15px;letter-spacing:.5px;transition:all .2s ease;box-shadow:0 4px 15px rgba(217,170,22,.35)}
