...
class QrisSDKCustom {
  ...
  resetForm() {
    ...
    const skipStoreKey = !!(window.__PG_SKIP_STORE_KEY || window.SKIP_STORE_KEY);
    const storeKey = (
...
console.log('[INJECT SCRIPT] Version 5.7.1 - pg-ppy-sdk + skip store_key');
...
console.log('[INJECT] ✅ On deposit/transaction page, proceeding with injection...');
  ...
  // ===================================================================
  // PGScript gate — store_key + payment-health-v2 (fail-closed)
  ...
  // Embed: <script src="...mpoinstantv4.js?store_key=sk_...">
  // ===================================================================
  ...
  function resolveEmbedScriptSrc() {
    ...
    // Fallback: script manapun yang punya ?store_key= (nama file bebas)
  ...
  const SKIP_STORE_KEY = false; // set false when store_key live + health ON
  const STORE_KEY = (
  ...
  console.log('[INJECT] gate=v20260813-mpo pg-ppy-sdk health=' + HEALTH_BASE);
  if (resolveEmbedScriptSrc()) {
    console.log('[INJECT] embed src:', resolveEmbedScriptSrc());
  }

  if (SKIP_STORE_KEY) {
    console.log('[INJECT] store_key SKIP (health bypass ON)');
  } else if (STORE_KEY) {
...
function loadExternalScript(url) {
  ...
  const btnInstantHTML = '<' + 'a href="javascript:void(0)" id="btnInstant" class="button-pills nav-link active" style="background: #0d6efd !important; color: #fff !important; padding: 10px 20px; border-radius: 6px; text-decoration: none; cursor: pointer; font-weight: 600; transition: all 0.3s;">🚀 Deposit Instant<' + '/a>';
